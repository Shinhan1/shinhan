...
public class board_QA_Dao {
	...
	public void QA_addData(board_QA_VO qa_vo) {
		...
		sb.append("values (board_qa_seq.nextval, ?, ?, ?, sysdate, ?, ?, ?, ?, 0, 0, 0) ");
	...
	// 전체 데이터 조회
	public ArrayList<board_QA_VO> getAllData(int startNo, int endNo) {
		ArrayList<board_QA_VO> list = new ArrayList<board_QA_VO>();
		
		
		sb.setLength(0);
		sb.append("select * ");
		sb.append("from (select rownum rn, qa_bno, qa_id, qa_title, qa_inquiry, qa_date, qa_contents, qa_email, qa_img, qa_ip, qa_status, qa_hits ");
	...
	// 총 게시물 파악 메소드
		...
		public board_QA_VO getData(int bno) {
			...
			try {
				...
				vo = new board_QA_VO(bno, id, title, inquiry, date, contents, email, img, ip, status, hits);
